-- El modal de bunques de tamizado (y otras pantallas de stock de lotes de
-- origen) solo tenían field_batch_code (código interno largo, ej.
-- NORA-20260714-182736-452657) y guia_remision (documento de la remisión
-- completa) para identificar un lote. Faltaba el comprobante de acopio
-- (exit_items.item_document_number), que es el número que efectivamente
-- reconoce el equipo de planta -- se expone acá para que el frontend deje
-- de mostrar el código interno.
...
create or replace view "public"."v_plant_available_stock" as  SELECT ei.id AS exit_item_id,
  ...
  GROUP BY ei.id, ei.production_batch_id, pb.batch_code, p.first_name, p.last_name, cm.name, eri.quantity_kg_received, qe.humidity_pct, qe.impurities_pct, qe.color, qe.sack_condition, qe.appearance, qe.approval_status, qe.rejected_kg, qe.rejection_reason, ei.cooperative_id, er.document_number, ei.item_document_number;
